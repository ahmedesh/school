...
class ReceiptStudentsRepository implements ReceiptStudentsRepositoryInterface
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $receipt_students->Debit = $request->Debit;  // الطالب دفعلي مبلغ كذا
            ...
            $fund_accounts->Debit = $request->Debit;  //امين الصندوق استلم الفلوس
            $fund_accounts->credit = 0.00;  // بس مطلعتش فلوس لطالب
            ...
            $fund_accounts->Debit = 0.00;  //الطالب مش عليه فلوس
            $fund_accounts->credit = $request->Debit;  //الطالب دفع اللي عليه او جزء من اللي عليه
            $fund_accounts->description = $request->description;
            $fund_accounts->save();
    ...
    {
        try {
            ReceiptStudent::destroy($request->id);  // لما يجذف الاب دا هيحذف الابناء تبعه
