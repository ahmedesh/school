            ...
            <div class="card card-statistics h-100">
                <div class="card-body">
                    ...
                    <button type="button" class="button x-small" id="btn_delete_all">
                        {{ trans('My_Classes_trans.delete_checkbox') }}  {{-- زرار حذف الصفوف المختارة --}}
                    </button>


                    <br><br>
{{-- Search البحث بالمرحله --}}
                    <form action="{{ route('Filter_Classes') }}" method="POST">
                        {{ csrf_field() }}
                        <select class="selectpicker" data-style="btn-info" name="Grade_id" required
                                onchange="this.form.submit()"> {{-- onchange="this.form.submit()"  => لو حصل اي تغيير روح عال action ونفذه--}}
                    ...
                    <div class="table-responsive">
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                            ...
                            @if (isset($details))  {{--$details => جايه من withDetails--}}

                                <?php $List_Classes = $details; ?>     {{-- لو عمل فلتره بالبحث  --}}
                            @else

                                <?php $List_Classes = $My_Classes; ?>  {{-- لو معملش فلتره بالبحث  --}}
                            @endif
    ...
    <div class="modal fade" id="delete_all" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('delete_all') }}" method="POST">  {{-- حذف جميع الصفوف --}}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        {{ trans('My_Classes_trans.Warning_Grade') }}
...
@section('js')
    ...
    {{-- ===================== كود ال checkbox ========================--}}
    <script>
        function CheckAll(className, elem) { // كود ال check يحددلي كله ويلغي تحديد كله لما اضغط عليه فقط
            var elements = document.getElementsByClassName(className);
            var l = elements.length;
            if (elem.checked) {  // لو عملت تشيك عليها حددلي كله
                for (var i = 0; i < l; i++) {
                    elements[i].checked = true;
                }
            } else {  // لو عملت تشيك عليها تاني الغي تحديد كله
                for (var i = 0; i < l; i++) {
    ...
    {{-- ============================= كود ال deleteAll متنزلش ال modael الا م يكون مختار صفوف بالفعل ================================== --}}
    <script type="text/javascript">
        $(function() {
            $("#btn_delete_all").click(function() {
                var selected = new Array();
                $("#datatable input[type=checkbox]:checked").each(function() {
                    selected.push(this.value);  // خد ال id اللي معلم عليهم معاك
                });

                if (selected.length > 0) {
                    $('#delete_all').modal('show')
                    $('input[id="delete_all_id"]').val(selected);  // هات ال id اللي معلم عليهم
                }
            });
        });
